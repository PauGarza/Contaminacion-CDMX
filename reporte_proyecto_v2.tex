...
\author{Paulina Garza Allende \and Andrea Monserrat Arredondo Rodr\'iguez \and Othoniel Camacho}
...
creciente en JAGS a trav\'es del paquete \texttt{R2jags}:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{output/figures/eda_valle_v2_scatter_ciudad.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{output/figures/diag_cadena_D_alpha_v2.png}
...
es el \emph{proceso gaussiano} (PG): una colecci\'on de variables aleatorias
...
\subsection{Predicci\'on temporal --- Cuarto trimestre de 2023}

El Modelo~C1 se us\'o tambi\'en para generar predicciones del promedio
mensual de PM2.5 sobre las 14~estaciones durante el cuarto trimestre de
2023, evaluando el predictor en las covariables medias de cada mes.
La distribuci\'on predictiva posterior muestra un patr\'on claramente
ascendente (Tabla~\ref{tab:q4}), consistente con el m\'aximo invernal
capturado por los t\'erminos de Fourier.

\begin{table}[H]
\centering
\caption{Predicci\'on de PM2.5 promedio mensual (14~estaciones)
         para el cuarto trimestre de 2023.
         IC: intervalo de credibilidad al 95\,\%.}
\label{tab:q4}
\begin{tabular}{lrrr}
\toprule
Mes & Media ($\mu$g/m$^3$) & IC 2.5\,\% & IC 97.5\,\% \\
\midrule
Octubre  & 13.41 & 12.42 & 14.45 \\
Noviembre & 16.11 & 14.82 & 17.46 \\
Diciembre & 17.29 & 16.08 & 18.57 \\
\bottomrule
\end{tabular}
\end{table}

Diciembre de 2023 (17.3~$\mu$g/m$^3$) supera la gu\'ia anual de la OMS
(15~$\mu$g/m$^3$), reflejando el deterioro de la calidad del aire en
temporada de bajas temperaturas y alta inversi\'on t\'ermica.
Los intervalos de credibilidad se ampl\'ian de octubre a diciembre
(de 2.0 a 2.5~$\mu$g/m$^3$), lo que refleja la mayor variabilidad
interdiaria propia del invierno.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{output/figures/diag_cadena_F1_beta1_v2.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{output/figures/diag_cadena_F2_gamma1_v2.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{output/figures/validacion_F1_imputacion_v2.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{output/figures/serie_mu_cdmx_v2.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{output/figures/efectos_F2_estaciones_v2.png}
...
\begin{enumerate}
  ...
  \item de Finetti, B. (1937). La pr\'evision: ses lois logiques, ses sources
        subjectives. \textit{Annales de l'Institut Henri Poincar\'e}, 7, 1--68.
...
\begin{itemize}
  ...
  \item \texttt{scripts/limpieza\_valle\_mexico\_v2.R} --- procesamiento y filtrado
  ...
  \item \texttt{scripts/modelo\_E\_valle\_v2\_profesor.R} --- Modelo E (PG espacial)
...
\noindent
\textbf{Nota sobre notaci\'on:} En JAGS, el segundo argumento de
\texttt{dnorm} es la \emph{precisi\'on} $\tau = 1/\sigma^2$
(no la varianza).
As\'i, \texttt{dnorm(0, 0.001)} corresponde a $\mathcal{N}(0, 1000)$
en notaci\'on est\'andar de varianza, es decir, una distribuci\'on
inicial pr\'acticamente plana sobre los coeficientes.
